...
public class DynamicVectorIntegers {
	...
	public DynamicVectorIntegers(DynamicVectorIntegers dVI) {
		data = new int[dVI.length()];
	...
	public boolean equals(Object v) {
		if (!(v instanceof DynamicVectorIntegers))
			return false;
		DynamicVectorIntegers dVI = (DynamicVectorIntegers) v;
		if (this.length() != dVI.length())
			return false;
		for (int i = 0; i < data.length; i++) {
			if (this.get(i) != dVI.get(i))
				return false;
		}
		return true;
	}
	...
	public DynamicVectorIntegers onlyEven() {
		DynamicVectorIntegers dVI = new DynamicVectorIntegers();
		for (int i = 0; i < length(); i++) {
			if (get(i) % 2 == 0)
				dVI.add(get(i));
		}
