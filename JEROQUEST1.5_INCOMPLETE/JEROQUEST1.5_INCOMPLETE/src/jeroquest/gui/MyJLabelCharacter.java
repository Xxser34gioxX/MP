...
/**
 * Class that models a graphical label of Java with a character associated to it
 * Having the character allows it to show the "life bar" over the icon
 * 
 * @author Programming Methodology Professors
 */
public class MyJLabelCharacter extends JLabel {
	...
	private void lifeBar(Graphics g, int currentValue, int maximum) {
		...
		if (character.getBody() > 0.75 * character.getMaxBody())
			g.setColor(Color.GREEN);
		// YELLOW: if it has between (50%..75%] of life
		else if (character.getBody() > 0.5 * character.getMaxBody())
	...
	public void paintComponent(Graphics g) {
		...
		if (character.getPosition() != null) {
			...
			lifeBar(g, character.getBody(), character.getMaxBody());
		} else
